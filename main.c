...
void ADC_INIT(){
	//*******************
	ADC->CCR |= (1<<16);		//Set the prescalar
	//*******************
	ADC1->CR1 &= ~(1<<24);		//12 bit RES
	ADC1->CR1 &= ~(1<<25);		//12 bit RES
	ADC1->CR1 &= ~(1<<8);		//SCAN mode disable
	//*******************
	ADC1->CR2 |= (1<<1);		//Continuous Conversion mode enable
	ADC1->CR2 |= (1<<8);		//DMA for ADC
	ADC1->CR2 |= (1<<9);		//DMA Continuous Request enable
	ADC1->CR2 |= (1<<10);		//EOC after each conversion
	ADC1->CR2 &= ~(1<<11);		//Data Alignment RIGHT
	//*******************
	ADC1->SMPR2 |= (1<<5);		//
	ADC1->SMPR2 &= ~(1<<4);		//Sampling time of 100 cycles for channel 1
	ADC1->SMPR2 &= ~(1<<3);		//
	//*******************
	ADC1->SQR1 &= ~(1<<20);		//1 Conversion
	//*******************
	ADC1->SQR3 |= (1<<0);		//Channel 1 First Conversion
	//*******************
...
void DMA_INIT(){
	//*******************
	DMA2_Stream0->CR &= ~(7<<25);		//Channel 0 select
	...
	DMA2_Stream0->CR |= (1<<10);		//Memory Address Increment enable
	DMA2_Stream0->CR |= (1<<8);			//Circular mode enable
	//*******************
}
void DMA_START(){
	//*******************
	DMA2_Stream0->PAR = (uint32_t) &ADC1->DR;	//Peripheral address
	DMA2_Stream0->M0AR = (uint32_t)adcData;		//Memory address
	DMA2_Stream0->NDTR = 1;						//Transfer data size
	//*******************
	DMA2_Stream0->CR |= 1<<0;					//DMA enable
	//*******************
}
void GPIO_INIT(){
	//*******************
